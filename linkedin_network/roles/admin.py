...
from .utils import save_roles_from_dataframe, save_functions_from_dataframe
import pandas as pd
...
class FunctionAdmin(admin.ModelAdmin):
    ...
    list_display = ('connection', 'function', 'months_in_function', 'is_current')
    search_fields = ('connection__first_name', 'connection__last_name', 'function')
    list_filter = ('connection__first_name', 'function')


    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('upload-csv-functions/', self.admin_site.admin_view(self.upload_csv), name="upload-csv-functions"),
        ]
        return custom_urls + urls
    ...
    def upload_csv(self, request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    df = pd.read_csv(csv_file)
                    save_functions_from_dataframe(df)
