...
resource "aws_security_group" "ec2_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0", "130.226.161.115/32"] # Only allow HTTPS traffic from everywhere
  }
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0", "130.226.161.115/32"]
  }
  tags = {
...
resource "aws_instance" "web" {
  ami                    = "ami-010d6cc0713a0f95f" # Amazon Linux
